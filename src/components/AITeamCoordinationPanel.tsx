...
type PanelVariant = 'page' | 'embedded';
...
interface AITeamCoordinationPanelProps {
  variant?: PanelVariant;
  className?: string;
}

export const AITeamCoordinationPanel: React.FC<AITeamCoordinationPanelProps> = ({
  variant = 'page',
  className = '',
}) => {
  const { user } = useAuth();
  ...
  if (loading) {
    return (
      <div className={`w-full ${variant === 'page' ? 'py-10' : 'py-6'} ${className}`}>
...
export default AITeamCoordinationPanel;
