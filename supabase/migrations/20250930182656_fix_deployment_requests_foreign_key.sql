-- Fix foreign key constraint issue for ai_employee_deployment_requests table
-- The error indicates the constraint is referencing 'users' instead of 'auth.users'

-- First, drop the table if it exists with incorrect constraints
DROP TABLE IF EXISTS public.ai_employee_deployment_requests CASCADE;

-- Recreate the table with correct foreign key constraints
CREATE TABLE public.ai_employee_deployment_requests (
...
-- Create index
CREATE INDEX idx_ai_emp_deploy_req_user ON public.ai_employee_deployment_requests(user_id, created_at DESC);
